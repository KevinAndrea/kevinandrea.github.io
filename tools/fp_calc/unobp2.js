...
function stage3() {
	...
	var nhdr = document.createElement("h2");
	nhdr.innerHTML = "Testing (No Denormalized Support)";
	div.append(nhdr);
	
	var ntxt = document.createElement("p");
	ntxt.innerHTML = "Below is a simulation of how the floating point should be calculated if a Floating Point system did not have Denormalized Values (as is the case for this Project). Flip the bits to change the value.";
	div.append(ntxt);
...
function stage4() {
  ...
  if(!spcase) {
  	ntxt = document.createElement("p");
    if(rawval == 0) {
  	  ntxt.innerHTML = "Calculated value would be Denormalized. In this Project, we would use 0";
    }
    else { 
